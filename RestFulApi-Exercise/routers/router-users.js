...
Router.post('/', [authentication, authorization], async (req, res) => {
    ...
    if(isUnique) return res.status(400).send('There is already a user with that email');
...
Router.put('/:email', [authentication, authorization], async (req, res) => {
    ...
    const isUnique = await Users.findOne({email: value.email});
    if(isUnique.email !== req.params.email) return res.status(400).send('There is already a user with that email');

    const user = await Users.findOneAndUpdate({email: req.params.email},{$set: value });
...
Router.get('/:email', [authentication, authorization], async (req, res) => {
    const result = await Users.findOne({email: req.params.email}).select('name email isAdmin -_id');
