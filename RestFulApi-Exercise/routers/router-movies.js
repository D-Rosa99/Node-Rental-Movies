...
Router.post('/', [authentication, authorization], async (req, res) => {
    ...
    if(!genre) return res.status(400).send(`${value.genre} is not a valid genre element`);
...
Router.put('/:model', [authentication, authorization], async (req, res) => {
    ...
    if(!movie) return res.status(400).send(`${req.params.model} is not a valid movie element`);
...
Router.get('/:movieName', [authentication, authorization], async (req, res) => {
    const result = await Movies.findOne({name: req.params.movieName}).select('-_id -__v');
    if(!result) return res.status(404).send(`${req.params.movieName} is not a valid movie element`);
