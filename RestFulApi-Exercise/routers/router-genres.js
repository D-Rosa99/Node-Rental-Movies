...
Router.post('/', [authentication, authorization], async (req, res) => {
    ...
    if(isUnique) return res.status(400).send('There is already a genre with that name');
...
Router.put('/:name', [authentication, authorization], async (req, res) => {
    ...
    const genre = await Genres.findOneAndUpdate({name: req.params.name},{$set: value });
...
Router.get('/:name', [authentication, authorization], async (req, res) => {
    const result = await Genres.findOne({name: req.params.name}).select('name -_id');
