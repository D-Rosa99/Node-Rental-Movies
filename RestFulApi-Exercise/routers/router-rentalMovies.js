...
Router.get('/', authentication, async (req, res) => {
    ...
    .select('-__v')
    .populate(
        { path: 'movie', select: 'name price genre -_id', 
...
Router.post('/', [authentication, authorization], async (req, res) => {
    ...
    if(getMovie.length !== value.movie.length) return res.status(400).send(`One or more of the movie are no valid`);  

    value.movie = getMovie.map( movie => movie.id);
    value.customer = getCustomer._id;
    value.totalPrice = getMovie.reduce( ( accumlator, movie ) => accumlator += movie.price, 0);
...
Router.put('/:invalidId', [authentication, authorization], async (req, res) => {
    await RentalMovie.findOneAndUpdate({_id: req.params.invalidId},{$set: {'isValidBill': false} });
